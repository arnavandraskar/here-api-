...
class speed_limit:
    ...
    def get_data(self,coord = []):
        if list(self.avail) != list(coord):
            self.avail = coord
            api_url = "https://routematching.hereapi.com/v8/match/routelinks?apikey={key}&waypoint0={lat},{long}&mode=fastest;car&routeMatch=1&attributes=SPEED_LIMITS_FCn(FROM_REF_SPEED_LIMIT,TO_REF_SPEED_LIMIT)".format(key = self.key ,lat=coord[0],long = coord[1])
